...
class ClotureJournee
{
    ...
    public function remboursementCapital()
    {
        ...
        if (count($dataGetCreditCDF) != 0) {
            ...
            foreach ($response as $dataGetCreditCDF) {
                if ($dataGetCreditCDF->CodeMonnaie == "CDF") {
                    ...
                    if ($checkJourRetard) {
                        if ($soldeMembre < $checkJourRetard->montantRetard) {
        ...
        if (count($dataGetCreditUSD) != 0) {
            ...
            foreach ($response as $dataGetCreditUSD) {
                if ($dataGetCreditUSD->CodeMonnaie == "USD") {
                    ...
                    $checkJourRetard = JourRetard::where("NumDossier", $dataGetCreditUSD->NumDossier)->first();
                    if ($checkJourRetard) {
                        if ($soldeMembre < $checkJourRetard->montantRetard) {
        ...
        if (count($dataGetCreditCDF) != 0) {
            ...
            foreach ($response as $dataGetCreditCDF) {
                if ($dataGetCreditCDF->CodeMonnaie == "CDF") {
                    ...
                    if ($soldeMembre > 0 or $soldeMembre == 0) {
                        ...
                        if ($checkRaw) {
                            ...
                            if ($checkRaw->CapitalPaye == $dataGetCreditCDF->CapAmmorti) {
                                ...
                                if ($soldeMembre <= $dataGetCreditCDF->Interet) {
                                    ...
                                    if ($soldeMembre < $dataGetCreditCDF->Interet) {
                                        $checkJourRetard = JourRetard::where("NumDossier", $dataGetCreditCDF->NumDossier)->where("DateRetard", "=!", $dateSystem)->first();
                ...
                if ($dataGetCreditUSD->CodeMonnaie == "USD") {
                    ...
                    if ($soldeMembre > 0 or $soldeMembre == 0) {
                        ...
                        if ($soldeMembre < $dataGetCreditUSD->CapAmmorti) {
                            ...
                            if ($checkRaw) {
                                //ICI LA LOGIQUE SI IL AVAIT DEJA PAYER  LE CAPITAL ET CA NE RESTE QUE l'INTERET
                                //première verification est ce que le montant qu'il avait remboursé est égal au montant de remboursement attendu ?
                                if ($checkRaw->CapitalPaye == $dataGetCreditUSD->CapAmmorti and $checkRaw->InteretPaye < $dataGetCreditUSD->Interet) {
                                    //SI CETTE CONDITION EST VRAI CELA SIGNIFIE QUE CE L'INTERET QUI EST EN RETARD
                                    //si l'interêt payé est 0 est que le solde est inferieur ou egale au montant à rembourser pour l'interet 
                                    if ($soldeMembre <= $dataGetCreditUSD->Interet) {
                                        //SI LE SOLDE EST INFERIEUR A L'INTERET QU'ON ATTENT QUE LA PERSONNE PAYE
                                        if ($soldeMembre < $dataGetCreditUSD->Interet) {
                                            $checkJourRetard = JourRetard::where("NumDossier", $dataGetCreditUSD->NumDossier)->where("DateRetard", "!=", $dateSystem)->first();
                                    ...
                                    //SI IL AVAIT DEJA PAYE QUELQUE CHOSE EN INTERET MAIS CA N'A PAS ETE COMPLET 
                                    if ($checkRaw->InteretPaye > 0 and $soldeMembre > 0) {
                                    ...
                                    if ($checkRaw->CapitalPaye < $dataGetCreditUSD->CapAmmorti) {
                                        ...
                                        if (($soldeMembre + $checkRaw->CapitalPaye + $checkRaw->InteretPaye) <= $dataGetCreditUSD->CapAmmorti + $dataGetCreditUSD->Interet) {
                                            ...
                                            //INTERET 
                                            if ($soldeMembre > 0 and $soldeMembre > ($dataGetCreditUSD->Interet - $checkRaw->InteretPaye)) {
    ...
    private function Reclassement(
        ...
        try {
            ...
            $soldeRestant = DB::select('SELECT SUM(echeanciers.CapAmmorti) as soldeRestant from echeanciers where echeanciers.NumDossier="' . $NumDossier . '" and echeanciers.posted=!1 and echeanciers.statutPayement=!1 GROUP BY echeanciers.NumDossier')[0];
            $SoldeCreditRestantUSD = $soldeRestant->soldeRestant;
            $compteProvision = PorteFeuilleConfing::first();
            ...
            if ($nbrJourRetard) {
                ...
                if ($nbrJourRetard->NbrJrRetard > 30 and $nbrJourRetard->NbrJrRetard <= 60 and $nbrJourRetard->provision2 == 0) {
                    $checknombreJrRetard = JourRetard::where("NumDossier", $NumDossier)
                        ->where("NbrJrRetard", ">", 0)
                        ->where("DateRetard", "=", $dateSystem)
                ...
                if ($nbrJourRetard->NbrJrRetard > 60 and $nbrJourRetard->NbrJrRetard <= 90 and $nbrJourRetard->provision3 == 0) {
                ...
                if ($nbrJourRetard->NbrJrRetard > 90 and $nbrJourRetard->NbrJrRetard <= 180 and $nbrJourRetard->provision4 == 0) {
    ...
    private function IncrementerJourRetard($NumDossier, $dateSystem, $NumCompteEpargne, $NumCompteCredit)
    {
        ...
        try {
            ...
            $checknombreJrRetard = JourRetard::where("NumDossier", $NumDossier)->first();
    ...
    function createJourRetardEccount($NumAdherant, $dateSystem, $NumCompteEpargne, $NumCompteCredit, $NumDossier, $CodeAgence, $NomCompte, $codeMonnaie)
    {
        ...
        if ($codeMonnaie == 2) {
            if ($NumAdherant < 10) {
                $compteProvisionCDF = "380100000" . $NumAdherant . "202";
                $compteCreanceLitigieuseCDF = "390100000" . $NumAdherant . "202";
            } else if ($NumAdherant >= 10 && $NumAdherant < 100) {
                $compteProvisionCDF = "38010000" . $NumAdherant . "202";
                $compteCreanceLitigieuseCDF = "39010000" . $NumAdherant . "202";
            } else if ($NumAdherant >= 100 && $NumAdherant < 1000) {
                $compteProvisionCDF = "3801000" . $NumAdherant . "202";
                $compteCreanceLitigieuseCDF = "3901000" . $NumAdherant . "202";
            } else if ($NumAdherant >= 1000 && $NumAdherant < 10000) {
                $compteProvisionCDF = "3801000" . $NumAdherant . "202";
                $compteCreanceLitigieuseCDF = "390100" . $NumAdherant . "202";
            } else if ($NumAdherant >= 10000 && $NumAdherant < 100000) {
                $compteProvisionCDF = "38010" . $NumAdherant . "202";
                $compteCreanceLitigieuseCDF = "39010" . $NumAdherant . "202";
            } else if ($NumAdherant >= 100000 && $NumAdherant < 1000000) {
                $compteProvisionCDF = "3801" . $NumAdherant . "202";
                $compteCreanceLitigieuseCDF = "3901" . $NumAdherant . "202";
            } else {
                $compteProvisionCDF = "3801" . $NumAdherant . "202";
                $compteCreanceLitigieuseCDF = "3901" . $NumAdherant . "202";
            }
            $checkRawExist = JourRetard::where("NumDossier", $NumDossier)->first();
            ...
            // else {
            //     $checkJourRetard = JourRetard::where("NumDossier", $NumDossier)->where("DateRetard", "=", $dateSystem)->first();
            //     JourRetard::where("NumDossier", $NumDossier)->update([
            //         "NumcompteEpargne" => $NumCompteEpargne,
            //         "NumcompteCredit" => $NumCompteCredit,
            //         "NumCompteCreanceLitigieuse" => $compteCreanceLitigieuseCDF,
            //         "CompteProvision" => $compteProvisionCDF,
            //         "NumDossier" => $NumDossier,
            //         "NbrJrRetard" => $checkJourRetard->NbrJrRetard + 1,
            //         "DateRetard" => $dateSystem,
        ...
        } else if ($codeMonnaie == 1) {

            if ($NumAdherant < 10) {
                $compteProvisionUSD = "380000000" . $NumAdherant . "201";
                $compteCreanceLitigieuseUSD = "390000000" . $NumAdherant . "201";
            } else if ($NumAdherant >= 10 && $NumAdherant < 100) {
                $compteProvisionUSD = "38000000" . $NumAdherant . "201";
                $compteCreanceLitigieuseUSD = "39000000" . $NumAdherant . "201";
            } else if ($NumAdherant >= 100 && $NumAdherant < 1000) {
                $compteProvisionUSD = "3800000" . $NumAdherant . "201";
                $compteCreanceLitigieuseUSD = "3900000" . $NumAdherant . "201";
            } else if ($NumAdherant >= 1000 && $NumAdherant < 10000) {
                $compteProvisionUSD = "3800000" . $NumAdherant . "201";
                $compteCreanceLitigieuseUSD = "390000" . $NumAdherant . "201";
            } else if ($NumAdherant >= 10000 && $NumAdherant < 100000) {
                $compteProvisionUSD = "38000" . $NumAdherant . "201";
                $compteCreanceLitigieuseUSD = "39000" . $NumAdherant . "201";
            } else if ($NumAdherant >= 100000 && $NumAdherant < 1000000) {
                $compteProvisionUSD = "3800" . $NumAdherant . "201";
                $compteCreanceLitigieuseUSD = "3900" . $NumAdherant . "201";
            } else {
                $compteProvisionUSD = "3800" . $NumAdherant . "201";
                $compteCreanceLitigieuseUSD = "3900" . $NumAdherant . "201";
            }
            ...
            if (!$checkRawExist) {
                ...
                $checkJourRetard = JourRetard::where("NumDossier", $NumDossier)->first();
                JourRetard::where("NumDossier", $NumDossier)->update([
                    "NumcompteEpargne" => $NumCompteEpargne,
                    "NumcompteCredit" => $NumCompteCredit,
                    "NumCompteCreanceLitigieuse" => $compteCreanceLitigieuseUSD,
