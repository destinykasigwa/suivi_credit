...
use App\Mail\ReponseDossierEmail;
use Carbon\Carbon;
...
class SendNotification
{
    ...
    public function sendEmailWhenResponseComment($data,$idUser,$idCredit,$senderName){
        //ENVOIE UN EMAIL
      $user = User::where("id", "=", $idUser)->first();
      $credit = Credits::where("id_credit", $idCredit)->first();
      $userName="Bonjour ". $user->name ." Vous avez recu une reponse au dossier de credit de " . $credit->NomCompte .
                    " NumCompte " . $credit->NumCompte .
                    " envoyée par " .$senderName; 
      Mail::to($user->email)->send(new ReponseDossierEmail($data,$userName));
    }
}
