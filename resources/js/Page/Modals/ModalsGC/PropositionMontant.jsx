...
const PropositionMontant = ({
    ...
    const fetchHistorique = async () => {
        ...
        try {
            ...
            setMessage({ type: "error", text: "Impossible de charger l'historique" });
    ...
    try {
        ...
        Swal.fire({
            icon: 'error',
            title: 'Erreur',
            text: error.response?.data?.msg || error.message || "Erreur lors de la proposition",
            toast: true,
            position: 'top-end',
            showConfirmButton: false,
            timer: 3000,
            ...
            <div className="pm-container mt-3">
                ...
                <div className="pm-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                    <div className="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <h6 className="fw-bold mb-0" style={{ fontSize: "1.05rem" }}>
    ...
    style={{
        ...
        backgroundColor: showForm ? "#dc3545" : "transparent",
        color: showForm ? "white" : "#20c997",
        borderColor: showForm ? "#dc3545" : "#20c997",
        cursor: "pointer",
        boxShadow: showForm ? "0 4px 12px rgba(220, 53, 69, 0.3)" : "none",
    }}
    onMouseEnter={(e) => {
        if (showForm) {
            e.currentTarget.style.backgroundColor = "#c82333";
            e.currentTarget.style.boxShadow = "0 6px 18px rgba(220, 53, 69, 0.4)";
        } else {
...
>
            ...
            <div className="flex-shrink-0" style={{ position: "relative", zIndex: 2 }}>
                <div
                    ...
                    style={{
                        width: 40,
                        height: 40,
                        background: isLast ? '#20c997' : '#f0f2f5',
                        boxShadow: isLast ? '0 0 0 3px rgba(32,201,151,0.2)' : 'none'
                    }}
                >
                    <i className={`fas ${isLast ? 'fa-star text-white' : 'fa-user text-secondary'}`}></i>
                </div>
                {!isLast && (
                    <div
                        style={{
            ...
            <div className="flex-grow-1 pb-1">
                <div className="d-flex justify-content-between align-items-start flex-wrap">
                    <div className="d-flex align-items-center gap-2">
                        ...
                        {isOwner && (
                            <button
                                onClick={() => handleDelete(item.id, item.montant_propose, item.user?.name)}
                ...
                <div className="mt-1 d-flex align-items-center gap-2 flex-wrap">
                    <span className="fw-semibold" style={{ fontSize: "0.85rem", color: "#6c757d" }}>
                        Montant proposé :
                    </span>
                    <span className="fw-bold text-teal" style={{ fontSize: "1.1rem" }}>
                    ...
                    {isLast && (
                        <span className="badge bg-teal-soft text-teal rounded-pill px-2 py-0 ms-1 pm-badge-last">
                            <i className="fas fa-clock me-1"></i>Dernière
                        </span>
                    )}
                </div>
                ...
                {item.commentaire && (
                    <div className="mt-2 p-2 rounded-3" style={{ background: '#fff8e1', borderLeft: '3px solid #ffc107' }}>
                        <div className="d-flex gap-1 align-items-start">
                            <span className="fw-semibold" style={{ fontSize: "0.8rem", color: "#856404" }}>
                                Commentaire: 
                            </span><br />
                        ...
                        <div className="text-center py-4">
                            ...
                            <small className="text-muted">Soyez le premier à proposer un montant</small>
                        </div>
                    )}
    ...
    {/* Montant proposé */}
    <div className="mb-4">
        <label className="form-label fw-semibold" style={{ color: '#2c3e50', fontSize: '0.95rem' }}>
            <i className="fas fa-money-bill-wave me-2" style={{ color: '#20c997' }}></i>
            Montant proposé <span className="text-muted fw-normal ms-1 " ><i className="text-danger">*</i></span>
        </label>
        ...
        <div className="text-end mt-1">
            <small className="text-muted" style={{ fontSize: '0.75rem' }}>
                <i className="fas fa-info-circle me-1"></i>Entrez le montant en chiffres
            </small>
        </div>
    </div>
    ...
    <div className="mb-4">
        <label className="form-label fw-semibold" style={{ color: '#2c3e50', fontSize: '0.95rem' }}>
            <i className="fas fa-comment-dots me-2" style={{ color: '#20c997' }}></i>
            Justification
            <span className="text-muted fw-normal ms-1 " ><i className="text-danger">*</i></span>
        </label>
        <textarea
        ...
        <div className="d-flex justify-content-between mt-1">
            <small className="text-muted" style={{ fontSize: '0.75rem' }}>
                <i className="fas fa-lightbulb me-1"></i>Ex: "Conforme aux directives", "Nécessite approbation"
            </small>
    ...
    <div className="mb-4">
        <label className="form-label fw-semibold" style={{ color: '#2c3e50', fontSize: '0.95rem' }}>
            <i className="fas fa-file-signature me-2" style={{ color: '#20c997' }}></i>
            Signature
            <span className="text-muted fw-normal ms-1 " ><i className="text-danger">*</i></span>
        </label>
        <div
            ...
            style={{
                ...
                cursor: 'pointer',
                transition: 'all 0.2s',
                background: signatureFile ? 'rgba(32,201,151,0.04)' : '#fafafa'
            }}
        >
            <input
                ref={fileInputRef}
            ...
            {signatureFile ? (
                    ...
                    {signaturePreview && (
                        <>
                            {signatureFile.type.startsWith('image/') ? (
                                ...
                                />
                            ) : signatureFile.type === 'application/pdf' ? (
                                <div className="pm-preview-pdf w-100 d-flex align-items-center justify-content-between p-3" style={{ background: '#f8f9fa', borderRadius: '8px' }}>
                                    <div className="d-flex align-items-center gap-3">
                                        <i className="fas fa-file-pdf text-danger fa-3x"></i>
                                        <div className="text-start">
                                            <p className="mb-0 fw-semibold">{signatureFile.name}</p>
                                            <small className="text-muted">{(signatureFile.size / 1024).toFixed(1)} Ko</small>
                                        </div>
                                    </div>
                                    <button
                                        type="button"
                                        className="btn btn-sm btn-outline-secondary"
                                        onClick={(e) => { e.stopPropagation(); window.open(signaturePreview, '_blank'); }}
                                    >
                ...
                <>
                    ...
                    <p className="mb-0 small text-muted">
                        <span className="fw-semibold">Cliquez pour charger</span> ou glissez‑déposez
                    </p>
                    <small className="text-muted" style={{ fontSize: '0.7rem' }}>JPG, PNG, GIF (max 5 Mo)</small>
                </>
            )}
        </div>
    </div>
