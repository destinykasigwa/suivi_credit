...
const PropositionsHistory = ({ propositionId, NumDossier, onClose = null }) => {
    ...
    const fetchPropositions = async () => {
        ...
        try {
            ...
            if (response.data.status === 1) {
                ...
                setSignaturesStats(response.data.signatures_stats || null);
            } else {
    ...
    const handleExportXLSX = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, `Propositions_Dossier_${NumDossier}`);
        
        // Télécharger le fichier
        XLSX.writeFile(wb, `Propositions_Dossier_${NumDossier}.xlsx`);
    };

    // ✅ Export PDF avec en-tête de l'organisation
    const handleExportPDF = () => {
        ...
        doc.text(organisation.adresse, pageWidth / 2, 30, { align: "center", maxWidth: 170 });
        doc.text(`${organisation.telephone} | ${organisation.email}`, pageWidth / 2, 36, { align: "center" });
        ...
        doc.setFontSize(14);
        doc.setFont("helvetica", "bold");
        doc.text("Historique des propositions de montant", pageWidth / 2, 58, { align: "center" });
        
        doc.setFontSize(10);
        doc.setFont("helvetica", "normal");
        doc.setTextColor(100, 100, 100);
        doc.text(`Dossier N°: ${NumDossier || propositionId}`, pageWidth / 2, 66, { align: "center" });
        
        // Date d'export
        doc.setFontSize(8);
        doc.setTextColor(150, 150, 150);
        doc.text(`Exporté le: ${new Date().toLocaleString("fr-FR")}`, pageWidth - 20, 75, { align: "right" });
        
        // Préparer les données pour le tableau
        const tableData = propositions.map((prop, index) => [
            (index + 1).toString(),
        ...
        doc.autoTable({
            startY: 80,
            head: [["#", "Signataire", "Rôle", "Montant", "Commentaire", "Date"]],
            body: tableData,
            theme: 'striped',
            headStyles: {
        ...
        if (signaturesStats) {
            ...
            doc.setFontSize(9);
            doc.setFont("helvetica", "normal");
            doc.text(`• Signatures effectuées: ${signaturesStats.total_signatures}/${signaturesStats.total_intervenants}`, 20, finalY + 7);
            doc.text(`• Pourcentage d'avancement: ${signaturesStats.pourcentage_avancement}%`, 20, finalY + 14);
            doc.text(`• Reste à signer: ${signaturesStats.reste_a_signer} signature(s)`, 20, finalY + 21);
            // if (signaturesStats.prochaine_signature) {
            //     doc.text(`• Prochain signataire: ${signaturesStats.prochaine_signature}`, 20, finalY + 28);
            // }
        }
        ...
        // Sauvegarder le PDF
        doc.save(`Historique_Propositions_Dossier_${NumDossier || propositionId}.pdf`);
    };
    ...
    if (error) {
        return (
            <div style={{ padding: "40px", textAlign: "center", backgroundColor: "#fef2f2", borderRadius: "16px" }}>
                <p style={{ color: "#dc2626" }}>{error}</p>
                <button onClick={fetchPropositions} style={{ marginTop: "16px", padding: "8px 20px", backgroundColor: "#3b82f6", color: "white", border: "none", borderRadius: "8px", cursor: "pointer" }}>
                    Réessayer
                </button>
            </div>
        );
    ...
    return (
        ...
        >
            ...
            {signaturesStats && (
                <div style={{ marginBottom: "24px", padding: "16px", backgroundColor: "#f8fafc", borderRadius: "16px", border: "1px solid #e5e7eb" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                        <div>
                            ...
                            <span style={{ fontSize: "12px", color: "#6b7280", marginLeft: "8px" }}>
                                ({signaturesStats.total_signatures}/{signaturesStats.total_intervenants} intervenants)
                            </span>
                        </div>
                    ...
                    {signaturesStats.reste_a_signer > 0 && (
                        <p style={{ fontSize: "11px", color: "#f59e0b", marginTop: "8px", marginBottom: 0 }}>
                            ...
                            En attente de {signaturesStats.reste_a_signer} signature{signaturesStats.reste_a_signer > 1 ? "s" : ""}
                            {/* {signaturesStats.prochaine_signature && ` (Prochain: ${signaturesStats.prochaine_signature})`} */}
                        </p>
                    )}
                </div>
            )}
            ...
            {dataToShow.length > 0 ? (
                <>
                        ...
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                            <thead>
                                <tr style={{ backgroundColor: "#f9fafb", borderBottom: "2px solid #e5e7eb" }}>
                                    ...
                                    <th style={{ textAlign: "right", padding: "14px 12px", fontWeight: "600" }}>
                                    ...
                                    <th style={{ textAlign: "center", padding: "14px 12px", fontWeight: "600" }}>
                                        Date
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                {dataToShow.map((proposition, index) => {
                                    const isLatest = index === dataToShow.length - 1;
                                    const roleColor = getRoleColor(proposition.role);
                                    const montant = proposition.montant_propose || proposition.montant || 0;
                                    const currencySymbol = getCurrencySymbol(proposition.devise || "CDF");
                                        ...
                                        >
                                            <td style={{ padding: "14px 12px", verticalAlign: "top" }}>
                                                <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                                                    <span style={{ fontWeight: "600", color: "#1f2937", display: "flex", alignItems: "center", gap: "6px" }}>
                                                        {proposition.nom || proposition.signed_by}
                                                        {isLatest && (
                                                            <span style={{ backgroundColor: "#10b981", color: "white", fontSize: "9px", padding: "2px 8px", borderRadius: "20px" }}>
                                                                <MdStar size={10} /> Dernière
                                                            </span>
                                                        )}
                                                    </span>
                                                    <span style={{ fontSize: "11px", color: roleColor, backgroundColor: `${roleColor}10`, padding: "2px 8px", borderRadius: "20px", display: "inline-block", width: "fit-content" }}>
                                                        {proposition.role}
                                                    </span>
                                                </div>
                                            </td>
                                            <td style={{ padding: "14px 12px", textAlign: "right", verticalAlign: "middle" }}>
                                                <span style={{ fontWeight: "700", fontSize: "15px", color: isLatest ? "#10b981" : "#1f2937" }}>
                                                    {formatCurrency(montant)} {currencySymbol}
                                                </span>
                                            </td>
                                            ...
                                            <td style={{ padding: "14px 12px", textAlign: "center", verticalAlign: "middle" }}>
                                                {formatDate(proposition.created_at || proposition.date)}
                                            </td>
                                        </tr>
                                    );
                                })}
                    ...
                    {/* Pied de tableau */}
                    <div style={{ marginTop: "20px", padding: "16px", backgroundColor: "#f9fafb", borderRadius: "12px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
                        <div style={{ display: "flex", gap: "24px", flexWrap: "wrap" }}>
                            <div>
                                <span style={{ fontSize: "11px", color: "#6b7280" }}>Nombre de propositions</span>
                                <div style={{ fontWeight: "700", fontSize: "18px", color: "#1f2937" }}>{dataToShow.length}</div>
                            </div>
                            <div>
                                <span style={{ fontSize: "11px", color: "#6b7280" }}>Montant final proposé</span>
                                <div style={{ fontWeight: "700", fontSize: "18px", color: "#10b981" }}>
                                    {formatCurrency(dataToShow[dataToShow.length - 1]?.montant_propose || 0)}{" "}
                                    {getCurrencySymbol(dataToShow[dataToShow.length - 1]?.devise || "CDF")}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div style={{ textAlign: "center", padding: "60px 20px" }}>
                    ...
                    <h3 style={{ fontSize: "18px", color: "#374151", marginBottom: "8px" }}>Aucune proposition</h3>
                    <p style={{ fontSize: "14px", color: "#6b7280" }}>Aucune proposition n'a été trouvée pour ce dossier</p>
                </div>
            )}
        </div>
    );
